...
class CharacterMotorMovement {
	...
	static var maxForwardSpeed : float = 7.0;
	var maxSidewaysSpeed : float = 7.0;
	var maxBackwardsSpeed : float = 7.0;
...
class CharacterMotorJumping {
	...
	var baseHeight : float = 2.8;
	
	// We add extraHeight units (meters) on top when holding the button down longer while jumping
	var extraHeight : float = 3.5;
...
function Update () {
	if (!useFixedUpdate)
		...
		if (Input.GetButtonUp ("Fire1")){
			CharacterMotorMovement.maxForwardSpeed = 7.0;
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded) {
		...
		jumpMessage = false; //groundMessage
		if (jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2)) {
			jumpMessage = true; // jumpMessage
			grounded = false;
