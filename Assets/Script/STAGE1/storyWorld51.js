...
function Update () 
{
	...
	if(talkMode)
	{
        ...
        {
            if (i == 0) text = "Through continuous exploration and hard work";
            if (i == 1) text = "Sunglasses finally came to this place";
            if (i >= 2) { text = "..."; obj.transform.position = Vector3.Lerp(obj.transform.position, target.position, Time.deltaTime * 5); }
        }
		else
		{
		    if (i == 0) text = "通过不断的探索与努力";
		    if (i == 1) text = "只会跳跃的墨镜君终于来到了这个地方";
		    if (i >=2) {text = "...";obj.transform.position = Vector3.Lerp (obj.transform.position, target.position,Time.deltaTime * 5);}
		}
