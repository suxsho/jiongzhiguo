...
var player2: GameObject;                        //拖入角色形象

//特别模式计分
var thisTimeHigh : int;

//让跳跃撞顶的声音只响一次
var crshTopSoundCheck: boolean = false;
function Start () 
{
    gameConfig.pauseMode = false;
    anim = GetComponent("Animator");           //载入动画

    saveGrav = gravity;                         //存储重力
}
...
function OnGUI()
{
    ...
    if(cancontrol && !gameConfig.pauseMode && SceneManager.GetActiveScene().name != "stage15"  && SceneManager.GetActiveScene().name != "title2"
        ...
        if(GUI.Button (Rect (880 * m_fScaleWidth, -28 * m_fScaleHeight, 0 * m_fScaleWidth, 0 * m_fScaleHeight), "" ))
        gamePause();
...
function Update () 
{
    ...
    if(Input.touchCount > 0)
    {
        ...
        if (touchMode1 == 1) {
            if (touch0.position.x < 140 * m_fScaleWidth && touch0.position.y < 250 * m_fScaleHeight) {
                ...
                touchMove = 0;
            //跳跃
            if (touch1.position.x > Screen.width - 200 * m_fScaleWidth && touch1.position.y < 250 * m_fScaleHeight)
                switch (touch1.phase) {
        ...
        if (touchMode2 == 1) {
            if (Input.touchCount > 1 && touch1.position.x < 140 * m_fScaleWidth && touch1.position.y < 250 * m_fScaleHeight) {
                touchMove = -1;
            ...
            else if (Input.touchCount > 1 && touch1.position.x > 140 * m_fScaleWidth && touch1.position.x < 180 * m_fScaleWidth)
                touchMove = 0;
            //防止玩家按跳后离开键盘角色一直前进问题
            else if (Input.touchCount <= 1 && touch0.position.x > Screen.width - 200 * m_fScaleWidth && touch0.position.y < 250 * m_fScaleHeight)
                touchMove = 0; 
            //跳跃
            if (touch0.position.x > Screen.width - 200 * m_fScaleWidth && touch0.position.y < 250 * m_fScaleHeight)
                switch (touch0.phase) {
                    case TouchPhase.Began:
                        touchJumpDown = true;
                        touching = true;
    ...
    if (cancontrol) {
        if (Input.GetAxis("Horizontal") > 0 || touchMove == 1) {
            player2.transform.localScale.x = 0.9;
        }
	...
	if(cancontrol)
	{
        ...
        { 
			 ...
			 runJumpEnable  = false;
             touchJumpUp = false;
             crshTopSoundCheck = false;                 //恢复声音判定
        ...
        {
            crashTop = true;
            if (!crshTopSoundCheck)
            {
                GetComponent.<AudioSource>().PlayOneShot(soundTop, 1.0F);
                crshTopSoundCheck = true;
            }
